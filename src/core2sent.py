...
def convert_coref(doc_etree):
    ...
    for entity_x in coref_x.findall('coreference'):
        ...
        entities.append(ent)
    entities.sort(key=lambda ent: ent['first_mention'])
...
def corexmls_from_files(linegen):
    for doc_i, filename in enumerate(linegen):
        if doc_i % 100 == 0:
            sys.stderr.write('.')
        fp = gzip.open(filename, 'rt', encoding='utf-8') if filename.endswith('.gz') else open(filename, 'r', encoding='utf-8')
        data = fp.read()
        s = filename
    ...
    for docid, data in corexmls_autodetect():
        ...
        if args.sentmode == 'dsent':
            ...
            print("{docid}\t{sent_texts}\t{fullinfo}".format(docid=docid,
                                                             ...
                                                             fullinfo=json.dumps({'sentences': sentences,
                                                                                  'entities': entities})))
            ...
            for sent_i, sent_info in enumerate(sentences):
                print("{docid}\t{sentid}\t{sent_text}\t{sent_info}".format(
                    ...
                    sent_info=json.dumps(sent_info)))
